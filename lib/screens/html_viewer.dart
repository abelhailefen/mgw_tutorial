...
class _HtmlViewerState extends State<HtmlViewer> {
  ...
  static const String baseUrl = "https://lessonservice.mgwcommunity.com/";
  ...
  Future<void> _initHtmlViewer() async {
    ...
    try {
      ...
      print(
          'Is local URL (based on file:// prefix): $isLocalUrl (URL: ${widget.url})');
      ...
      if (isLocalUrl) {
        ...
        final hasConnection = connection != ConnectivityResult.none &&
            connection != ConnectivityResult.vpn; // Consider VPN as online
        ...
        if (!hasConnection) {
          ...
          setState(() {
            ...
            _controller =
                null; // Ensure no partial webview is shown without connection
          });
        ...
        final fullUrl = widget.url.startsWith('http')
            ? widget.url
            : '$baseUrl${widget.url.startsWith('/') ? '' : '/'}${widget.url}';
        ...
        await _loadOnlineHtml(fullUrl).timeout(
          ...
          onTimeout: () {
            ...
            print(
                'Timeout during online load initialization for URL: $fullUrl');
            if (mounted) {
              // Check mounted again inside timeout callback
              setState(() {
                _isLoading = false;
      ...
      if (mounted) {
        setState(() {
          ...
          _errorMessage =
              '${_l10n?.couldNotLoadItem ?? 'Failed to load content'}: $e';
          _controller = null; // Ensure no partial webview
        ...
        ..setNavigationDelegate(
          NavigationDelegate(
            ...
            onWebResourceError: (error) {
              print(
                  'WebView resource error for $url: ${error.errorCode}, ${error.description}');
              // An error occurred *within* the webview. Display the error but keep the controller.
              if (mounted) {
                setState(() {
                  _isLoading =
                      false; // Loading is definitely finished (with an error)
                  // Use available l10n key with fallback for general load error + description
                  _errorMessage =
                      '${_l10n?.couldNotLoadItem ?? 'Failed to load'}: ${error.description}';
                });
              }
            },
      ...
      if (mounted) {
        setState(() {
          ...
          _errorMessage =
              '${_l10n?.couldNotLoadItem ?? 'Failed to load content'}: $e';
          _controller =
              null; // Ensure no controller is used if load setup failed
    ...
    try {
      ...
      if (!await file.exists()) {
        ...
        if (mounted) {
          setState(() {
            ...
            _errorMessage =
                _l10n?.couldNotOpenFileError(path) ?? 'File not found: $path';
      ...
      final controller = WebViewController()
        ...
        ..setNavigationDelegate(
          NavigationDelegate(
            ...
            onWebResourceError: (error) {
              print(
                  'WebView resource error for local file $path: ${error.errorCode}, ${error.description}');
              if (mounted) {
                setState(() {
                  _isLoading = false;
                  // Use available l10n key with fallback, including path and description
                  _errorMessage = _l10n?.couldNotOpenFileError(
                          '$path: ${error.description}') ??
                      'Failed to open file: $path - ${error.description}';
      ...
      if (mounted) {
        setState(() {
          ...
          _errorMessage = _l10n?.couldNotOpenFileError('$path: $e') ??
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(
        context)!; // Guaranteed available after didChangeDependencies
    final isDarkMode = Theme.of(context).brightness == Brightness.dark;
    final dynamicPrimaryColor =
        isDarkMode ? AppColors.primaryDark : AppColors.primaryLight;
    final dynamicOnSurfaceColor = isDarkMode
        ? AppColors.onSurfaceDark
        : AppColors.onSurfaceLight; // Use AppColors for consistency
    final dynamicErrorColor =
        AppColors.error; // Assuming AppColors.error is theme-agnostic
      ...
      body: Stack(
        children: [
          ...
          if (_errorMessage != null)
            Center(
              child: Container(
                // Wrap in Container for padding/margins if needed
                ...
                child: Column(
                  ...
                  children: [
                    Icon(Icons.error_outline,
                        color: dynamicErrorColor, size: 50),
                    const SizedBox(height: 16),
                    Text(
                      _errorMessage!,
                      textAlign: TextAlign.center,
                      style:
                          TextStyle(color: dynamicOnSurfaceColor, fontSize: 16),
                    ),
                    const SizedBox(height: 24),
                    ElevatedButton(
                      onPressed: _initHtmlViewer, // Provide a retry mechanism
                      child: Text(
                          l10n.refresh), // Use existing 'refresh' key for retry
                    ),
                  ],
                ),
              ),
