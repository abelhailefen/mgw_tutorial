...
class SectionProvider with ChangeNotifier {
  ...
  Future<void> fetchSectionsForCourse(int courseId, {bool forceRefresh = false}) async {
    ...
    print("SectionProvider: fetchSectionsForCourse called for course $courseId (forceRefresh: $forceRefresh)");

    // Set loading state early if there's no data yet or force refreshing
    bool hasExistingData = _sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty;
    ...
    if (showLoadingIndicator) {
       ...
       print("SectionProvider: Showing cached data while fetching for course $courseId. No initial loading indicator.");
    }
    ...
    // (in case the data in the map is stale compared to DB).
     if(!hasExistingData || forceRefresh) {
        print("SectionProvider: Attempting to load sections for course $courseId from DB...");
        try {
           final List<Map<String, dynamic>> sectionMaps = await _dbHelper.query(
             'sections',
             where: 'courseId = ?',
             whereArgs: [courseId],
             orderBy: "'order' ASC, title ASC", // Add secondary sort for consistency
           );
           ...
           if (loadedSections.isNotEmpty) {
             ...
             print("SectionProvider: Loaded ${loadedSections.length} sections for course $courseId from DB. Notifying listeners.");
             notifyListeners(); // Notify to show cached data immediately
           } else {
             _sectionsByCourseId[courseId] = []; // Ensure the key exists, even if empty
             print("SectionProvider: No sections found in DB for course $courseId.");
           }
        } catch (e) {
           print("SectionProvider: Error loading sections for course $courseId from DB: $e");
           // Keep _sectionsByCourseId[courseId] potentially null or empty if DB load failed
           _sectionsByCourseId[courseId] = []; // Ensure key exists to avoid null checks later
        }
     }
    ...
    // The refresh indicator or initial load handles the loading state.
     if (forceRefresh || !hasExistingData) {
       ...
       try {
          ...
          _isLoadingForCourseId[courseId] = false; // Always turn off loading when fetch finishes
          notifyListeners(); // Ensure UI updates after network attempt, even if data didn't change or failed
          print("SectionProvider: Fetch process finished for course $courseId. Notified listeners. Error: ${_errorForCourseId[courseId]}. Sections count: ${_sectionsByCourseId[courseId]?.length}");
       }
     } else {
        print("SectionProvider: Not forcing refresh and existing data is available for course $courseId. Skipping network fetch.");
  ...
  Future<void> _performNetworkFetch(int courseId) async {
    ...
    print("SectionProvider: Attempting network fetch for course $courseId from $url");

    try {
      final response = await http.get(url, headers: {"Accept": "application/json"})
                                .timeout(const Duration(seconds: 20)); // Shorter timeout for sections? Or maybe 45 is fine.

      print("SectionProvider: Sections API Response for course $courseId Status: ${response.statusCode}");
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedBody is List) {
           ...
           print("SectionProvider: API returned a list directly.");
            ...
            // This is just a guess based on the course API, adjust if section API differs
             if (decodedBody is Map<String, dynamic> && decodedBody.containsKey('sections') && decodedBody['sections'] is List) {
                extractedData = decodedBody['sections'];
                 print("SectionProvider: API returned object with 'sections' key.");
             } else {
                String parseError = 'Failed to load chapters: Unexpected API response format.';
                ...
                if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
                    _errorForCourseId[courseId] = parseError;
                     ...
                     print("SectionProvider: Network parse error after showing cached data for course $courseId.");
        ...
        final List<Section> fetchedSections = extractedData
              .map((sectionJson) {
                 try {
                   return Section.fromJson(sectionJson as Map<String, dynamic>);
                    ...
                    print("SectionProvider: Error parsing individual section JSON: ${e.runtimeType}: $e");
                    print("Stacktrace: $s");
           ...
           if (!listEquals(currentSections, fetchedSections)) {
              print("SectionProvider: Network data for course $courseId is different. Saving and updating state.");
             ...
             print("SectionProvider: Network data for course $courseId is the same as current. No update needed.");
             _errorForCourseId[courseId] = null; // Clear any previous network error if fetch was successful
           }
      ...
      print("SectionProvider: TimeoutException fetching sections for course $courseId: $e");
      // Only set error if there's no cached data being displayed
       if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
         _errorForCourseId[courseId] = _timeoutErrorMessage;
       } else {
          print("SectionProvider: Network timeout after showing cached data for course $courseId.");
         _errorForCourseId[courseId] = null; // Keep cached data, no critical error banner
       }
    } on SocketException catch (e) {
      print("SectionProvider: SocketException fetching sections for course $courseId: $e");
      // Only set error if there's no cached data being displayed
       if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
         _errorForCourseId[courseId] = _networkErrorMessage;
       } else {
          print("SectionProvider: Network socket error after showing cached data for course $courseId.");
         _errorForCourseId[courseId] = null; // Keep cached data, no critical error banner
       }
    } on http.ClientException catch (e, s) { // Added stacktrace capture
      print("SectionProvider: ClientException fetching sections for course $courseId: ${e.message}");
      print("Stacktrace: $s"); // Print stacktrace
      // Only set error if there's no cached data being displayed
      if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
         _errorForCourseId[courseId] = "${_networkErrorMessage}: ${e.message}";
       } else {
          print("SectionProvider: Network client error after showing cached data for course $courseId.");
         _errorForCourseId[courseId] = null; // Keep cached data, no critical error banner
       }
    }
    catch (e, s) { // Added stacktrace capture
      print("SectionProvider: Generic Exception during network fetch for course $courseId: $e");
      print("Stacktrace: $s"); // Print stacktrace
      // Only set error if there's no cached data being displayed
      if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
         _errorForCourseId[courseId] = "${_unexpectedErrorMessage}: ${e.toString()}";
       } else {
         print("SectionProvider: Generic network error after showing cached data for course $courseId.");
         _errorForCourseId[courseId] = null; // Keep cached data, no critical error banner
       }
    }
  }
   ...
   Future<void> _saveSectionsToDb(int courseId, List<Section> sectionsToSave) async {
     try {
        final db = await _dbHelper.database; // Get DB instance once for transaction
        await db.transaction((txn) async {
           print("SectionProvider: Clearing existing sections for course $courseId from DB in transaction...");
           await txn.delete('sections', where: 'courseId = ?', whereArgs: [courseId]);
           print("SectionProvider: Deleted existing sections for course $courseId.");

           if (sectionsToSave.isEmpty) {
             print("SectionProvider: No sections to save to DB for course $courseId.");
             return;
           }

           print("SectionProvider: Saving ${sectionsToSave.length} sections for course $courseId to DB in transaction...");
           for (final section in sectionsToSave) {
             // Use the prefix for ConflictAlgorithm
             await txn.insert('sections', section.toMap(), conflictAlgorithm: sqflite.ConflictAlgorithm.replace);
           }
           print("SectionProvider: Sections for course $courseId saved to DB successfully in transaction.");
        });
     } catch (e, s) {
       print("SectionProvider: Error saving sections for course $courseId to DB: $e\n$s");
    ...
    bool listEquals(List<Section> a, List<Section> b) {
      ...
      for (int i = 0; i < a.length; i++) {
         ...
         if (a[i].id != b[i].id || a[i].title != b[i].title || a[i].order != b[i].order) {
            return false;
         }
      }
      return true;
      ...
      if (errorBody is Map && errorBody.containsKey('message') && errorBody['message'] != null && errorBody['message'].toString().isNotEmpty) {
        ...
        if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
           _errorForCourseId[courseId] = errorBody['message'].toString();
            ...
            print("SectionProvider: Network API error after showing cached data for course $courseId: ${errorBody['message']}");
         ...
         if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
            ...
            print("SectionProvider: Network HTTP error after showing cached data for course $courseId (Status: ${response.statusCode}).");
            // No critical error banner needed as cached data is shown
         }
      }
    } catch (e) {
       ...
       if (!(_sectionsByCourseId.containsKey(courseId) && _sectionsByCourseId[courseId]!.isNotEmpty)) {
          _errorForCourseId[courseId] = "$defaultUserMessage (Status: ${response.statusCode}). Response not parsable.";
           ...
           print("SectionProvider: Network HTTP error (non-parsable body) after showing cached data for course $courseId (Status: ${response.statusCode}).");
            // No critical error banner needed as cached data is shown
       }
    }
  }
  ...
  void clearErrorForCourse(int courseId) {
    ...
    if (_errorForCourseId.containsKey(courseId) && _errorForCourseId[courseId] != null) {
