...
class LessonProvider with ChangeNotifier {
  ...
  List<Lesson> lessonsForSection(int sectionId) =>
      _lessonsBySectionId[sectionId] ?? [];
  bool isLoadingForSection(int sectionId) =>
  ...
  static const String _unexpectedErrorMessage =
      "An unexpected error occurred while fetching lessons. Please try again.";
  static const String _failedToLoadLessonsLabel =
      "Failed to load lessons for this section.";
  static const String _apiBaseUrl =
      "https://lessonservice.mgwcommunity.com/api";

  Future<void> fetchLessonsForSection(int sectionId,
      {bool forceRefresh = false}) async {
    await _loadLessonsFromDb(sectionId);
    if (_lessonsBySectionId.containsKey(sectionId) &&
        _lessonsBySectionId[sectionId]!.isNotEmpty) {
      await _checkExistingDownloads(_lessonsBySectionId[sectionId]!);
    }

    if (!forceRefresh &&
        (_lessonsBySectionId[sectionId]?.isNotEmpty ?? false) &&
        !(_isLoadingForSectionId[sectionId] ?? false)) {
      debugPrint(
          "LessonProvider: Data for section $sectionId already available and not forcing refresh.");
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (extractedData is List) {
          ...
          if (forceRefresh ||
              !listEquals(_lessonsBySectionId[sectionId], fetchedLessons)) {
          ...
          _errorForSectionId[sectionId] =
              'Failed to load lessons: Unexpected API response format.';
        }
      } else {
        _handleHttpErrorResponse(
            response, sectionId, _failedToLoadLessonsLabel);
      }
    } on TimeoutException {
      ...
      debugPrint(
          "LessonProvider: Unexpected error fetching lessons for section $sectionId: $e\n$s");
  ...
  Future<void> _loadLessonsFromDb(int sectionId) async {
    try {
      ...
      debugPrint(
          "LessonProvider: Loaded ${loadedLessons.length} lessons from DB for section $sectionId.");
    } catch (e, s) {
      debugPrint(
          "LessonProvider: Error loading lessons from DB for section $sectionId: $e\n$s");
    }
  }

  Future<void> _saveLessonsToDb(
      int sectionId, List<Lesson> lessonsToSave) async {
    ...
    try {
      ...
      debugPrint(
          "LessonProvider: Saved ${lessonsToSave.length} lessons to DB for section $sectionId.");
    } catch (e, s) {
      debugPrint(
          "LessonProvider: Error saving lessons to DB for section $sectionId: $e\n$s");
    }
  }

  Future<void> _checkExistingDownloads(List<Lesson> lessons) async {
    debugPrint(
        "LessonProvider: Checking existing downloads for ${lessons.length} lessons.");
    for (final lesson in lessons) {
      final downloadId = getDownloadId(lesson);
      if (downloadId != null) {
        await MediaService.isFileDownloaded(downloadId);
        debugPrint(
            "LessonProvider: Checked download status for lesson ${lesson.id} (ID: $downloadId). Status: ${MediaService.getDownloadStatus(downloadId).value}");
      }
    }
    notifyListeners();
  }
  ...
  Future<bool> startDownload(Lesson lesson) async {
    ...
    if (downloadId == null) {
      ...
      notifyListeners();
      debugPrint(
          "LessonProvider: Download ID is null for lesson ${lesson.id}. Cannot start download.");
      return false;
    }

    if (MediaService.getDownloadStatus(downloadId).value ==
    ...
    if (MediaService.getDownloadStatus(downloadId).value ==
      ...
      debugPrint(
          "LessonProvider: Download already in progress for ID $downloadId.");
      notifyListeners();
      return true;
    }
  ...
  Future<String?> getDownloadedFilePath(Lesson lesson) async {
    ...
    if (downloadId == null) {
      debugPrint(
          "LessonProvider: Cannot get downloaded file path, download ID is null for lesson ${lesson.id}.");
      return null;
    }
  ...
  Future<void> deleteDownload(Lesson lesson, BuildContext context) async {
    ...
    if (downloadId == null) {
      ...
      debugPrint(
          "LessonProvider: Cannot delete download, ID is null for lesson ${lesson.id}.");
      return;
    }
  ...
  Future<void> cancelDownload(Lesson lesson) async {
    ...
    if (downloadId != null) {
      ...
      debugPrint(
          "LessonProvider: Cannot cancel download, ID is null for lesson ${lesson.id}.");
    }
  }

  void _handleHttpErrorResponse(
      http.Response response, int sectionId, String defaultUserMessage) {
    try {
