...
class ExamProvider with ChangeNotifier {
  ...
  final String _baseApiUrl =
      "https://courseservice.mgwcommunity.com/api/exams/chapter/";
  final DatabaseHelper _dbHelper = DatabaseHelper();

  Future<void> fetchExamsForChapter(int chapterId,
    ...
    final cached = await _dbHelper
        .query('exams', where: 'chapterId = ?', whereArgs: [chapterId]);
    ...
    if (forceRefresh || cached.isEmpty) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (responseData.containsKey('data') &&
            ...
            List<Exam> fetchedExams =
                examsJson.map((json) => Exam.fromJson(json)).toList();
            ...
            _errorMessages[chapterId] =
                'Unable to load exams. Please try again later.';
            _examsCache[chapterId] = cached.isNotEmpty
                ? cached.map((e) => Exam.fromJson(e)).toList()
                : [];
          }
        } else {
          _errorMessages[chapterId] =
              'Unable to load exams. Please check your connection.';
          _examsCache[chapterId] = cached.isNotEmpty
              ? cached.map((e) => Exam.fromJson(e)).toList()
              : [];
        }
        ...
        if (e is SocketException || e is TimeoutException) {
          _errorMessages[chapterId] = cached.isNotEmpty
              ? null
