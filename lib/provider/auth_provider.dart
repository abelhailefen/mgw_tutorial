...
class AuthProvider with ChangeNotifier {
    ...
    try {
        ...
        if (savedSession != null && savedSession['user_id'] != null) {
           ...
           _currentUser = User(
             ...
             allCourses: (savedSession['all_courses'] as int? ?? 0) == 1, // SQLite INT to bool
             enrolledAll: (savedSession['enrolled_all'] as int? ?? 0) == 1, // SQLite INT to bool
             serviceType: savedSession['service_type'] as String? ?? 'Not Specified',
              // enrolledCourseIds: (jsonDecode(savedSession['enrolled_course_ids'] ?? '[]') as List).cast<int>(), // Decode JSON list if saved as JSON
           );

           print("AuthProvider: Loaded User ID ${_currentUser!.id} from DB. Considering user logged in (Offline Mode). Status: ${_currentUser!.status}.");

        } else {
          print("AuthProvider: No saved session found in DB or user_id is null. User is not logged in.");
           await logout(clearDb: true); // Ensure clean state if DB is empty or corrupt
        }
    } catch (e) {
        print("AuthProvider: FATAL Error during initialization (loading from DB): $e");
         ...
         _apiError = ApiError(message: "An error occurred while trying to restore your session from local data. Please log in.");
    }

    _isInitializing = false;
    notifyListeners();
    print("AuthProvider Initialization Finished. Current User: ${_currentUser?.id}");
  }
  ...
  String normalizePhoneNumberToE164(String rawPhoneNumber) {
    ...
    if (cleanedNumber.length == 9 && (cleanedNumber.startsWith('9') || cleanedNumber.startsWith('7'))) return '+251$cleanedNumber';
    ...
    // assume it might need a +251 prefix as a best guess, especially if it doesn't start with 0 or +251.
     if (cleanedNumber.length >= 9 && int.tryParse(cleanedNumber) != null) {
        print("Warning: Phone '$rawPhoneNumber' doesn't match standard ET patterns. Guessing +251 prefix.");
    ...
    print("Warning: Could not confidently normalize phone number '$rawPhoneNumber' to E.164. Returning cleaned: $cleanedNumber");
...
Future<bool> login({
    ...
    if (response.statusCode == 200) {
      ...
      try {
        ...
        if (authResponse.user == null) {
          ...
          _apiError = ApiError(
            message: authResponse.message?.isNotEmpty == true ? authResponse.message! : "Login failed: No user data returned by server.",
          );
          _isLoading = false;
          notifyListeners();
          return false;
        ...
        print("Login successful: User ID=${_currentUser?.id}. Status: ${_currentUser?.status}.");
        ...
        if (_currentUser != null && _currentUser!.id != null && _currentUser!.id! > 0) {
          ...
          print("AuthProvider: Login succeeded but invalid user ID in API response. Cannot establish logged-in state.");
           _apiError = ApiError(message: "Login failed: Invalid user data returned by server.");
           // Ensure _currentUser is null if the ID is invalid, even if API returned *something*.
           _currentUser = null;
           await _dbHelper.deleteLoggedInUser(); // Ensure local session is also clean
           _isLoading = false;
           notifyListeners();
  ...
  Future<void> logout({bool clearDb = true}) async {
    ...
    if (clearDb) {
      try {
         ...
         print("AuthProvider: Failed to clear user session from database on logout: $e");
  ...
  Future<bool> signUpSimple({
    ...
    String? firstName,
    String? lastName,
    String? languageCode, // Although not used in User model, keep for potential future use
    String? deviceInfo,
  }) async {
    ...
    final url = Uri.parse('$_apiBaseUrl/api/users'); // Assuming /api/users endpoint for simple signup
    ...
    try {
      ...
      print("AuthProvider: Simple sign up response - Status: ${response.statusCode}, Body: ${response.body}");
  ...
  Future<bool> registerUserFull({
    ...
    // Prepare the full payload using data from the User object passed in
     final Map<String, dynamic> fullRegistrationPayload = {
      ...
      'status': registrationData.status, // Use the status from the User object (likely "pending")
    ...
    final url = Uri.parse('$_apiBaseUrl/api/users'); // Assuming this is the endpoint for full user creation
    try {
      final body = json.encode(fullRegistrationPayload); // Encode the map directly
      final response = await http
          .post(
            url,
            headers: {'Content-Type': 'application/json'},
       ...
       print("AuthProvider: Full registration response - Status: ${response.statusCode}, Body: ${response.body}");
      ...
      if (response.statusCode == 200) {
         ...
         if (_currentUser != null && _currentUser!.id != null && _currentUser!.id! > 0) {
             try {
                ...
                print("AuthProvider: Failed to save user session to database after name change: $e");
                 // Set an error message about the local failure, but the API call succeeded.
                 _apiError = ApiError(message: "Name updated, but failed to save locally.");
        ...
        return {'success': true, 'message': 'Name updated successfully.'};
          ...
          print("AuthProvider: API authentication failed during name change (${response.statusCode}). Session likely invalid online.");
           ...
           String authErrorMessage = "Your online session appears invalid. Please log in again.";
         ...
         if (data.containsKey('user') && data['user'] != null) {
          ...
          if (_currentUser != null && _currentUser!.id != null && _currentUser!.id! > 0) {
              try {
                 ...
                 print("AuthProvider: Failed to save user session to database after phone change: $e");
                  _apiError = ApiError(message: "Phone number updated, but failed to save locally."); // Notify about DB save error
                  notifyListeners(); // Notify immediately about the local save error
              }
          }
  ...
  void _handleErrorResponse(http.Response response, String defaultMessage, {bool isLogin = false}) {
    ...
    try {
      ...
      _apiError = ApiError(
          message: message,
          errors: errors != null && errors.isNotEmpty ? errors : null // Only include errors if the list is not empty
      );
    } catch (e) {
